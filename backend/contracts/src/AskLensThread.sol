...
contract AskLensThread is ERC1155, Ownable {
    ...
    function mint(address alice, address quevedo, uint256 questionId, string memory tokenURI) external returns (uint256) {
        _tokenIds.increment();

        AskLensQuestion _AskLensQuestion = AskLensQuestion(AskLensQuestionContract);

        require(
            _AskLensQuestion.balanceOf(alice, questionId) > 0,
            "Can't answer a question that doesn't belong to you"
        );

        _AskLensQuestion.burn(questionId, quevedo, alice);
        ...
        emit QuestionAnswered(alice, quevedo, tokenId, tokenURI);
